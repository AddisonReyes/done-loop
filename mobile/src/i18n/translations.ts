export const translations = {
  en: {
    common: {
      off: 'Off',
      on: 'On',
      future: 'future',
      pending: 'Pending',
      yes: 'Yes',
      no: 'No',
      none: 'None',
      clear: 'Clear',
      close: 'Close',
      create: 'Create',
    },
    tabs: {
      habits: 'Habits',
      todos: 'Tasks',
      settings: 'Settings',
      calendar: 'Calendar',
    },
    habits: {
      title: 'Habits',
      description: 'Create daily habits, mark today’s progress, and remove what is no longer part of your routine.',
      completedSummary: 'completed',
      pendingSummary: 'pending',
      loading: 'Loading habits...',
      empty: 'You do not have active habits yet. Create the first one to start today’s loop.',
      emptyFilter: 'No habits in this filter.',
      loadError: 'Could not load habits.',
      saveError: 'Could not save habit changes.',
      formLabel: 'Habit name',
      formPlaceholder: 'New habit',
      descriptionLabel: 'Description',
      descriptionPlaceholder: 'Optional note',
      recurrence: 'Recurrence',
      customInterval: 'Every N days',
      customIntervalRequired: 'Enter a whole number greater than zero.',
      customIntervalUnit: 'days',
      everyDays: 'Every {{count}} days',
      weeklyDays: 'Days of the week',
      monthlyDays: 'Days of the month',
      addMonthlyDays: 'Select monthly days',
      monthlyDateLabel: 'Monthly date {{index}}',
      removeMonthlyDate: 'Remove monthly date {{index}}',
      reminder: 'Reminder',
      reminderTime: 'Reminder time',
      create: 'Create',
      createTitle: 'New habit',
      editTitle: 'Edit habit',
      editLabel: 'Edit habit',
      completed: 'Completed',
      completedToday: 'Completed today',
      pending: 'Pending',
      pendingToday: 'Pending today',
      notScheduledToday: 'Not scheduled today',
      save: 'Save',
      cancel: 'Cancel',
      edit: 'Edit',
      delete: 'Delete',
      archived: 'Archived',
      archivedHelp: 'Archived habits are inactive until you unarchive them.',
      deleteConfirmTitle: 'Delete habit?',
      deleteConfirmMessage: 'Choose whether to keep previous history or delete every entry for this habit.',
      deleteFromDate: 'Delete from this day forward',
      deleteAllHistory: 'Delete all history',
      detail: {
        interval: 'Interval',
        recurrence: 'Recurrence',
        days: 'Days',
        dates: 'Dates',
        reminder: 'Reminder',
        reminderTime: 'Time',
        status: 'Status',
      },
      filters: {
        all: 'All',
        pendingToday: 'Pending',
        completedToday: 'Done',
      },
      recurrences: {
        daily: 'Daily',
        weekly: 'Weekly',
        monthly: 'Monthly',
        custom: 'Custom',
      },
      weekdays: {
        1: 'Monday',
        2: 'Tuesday',
        3: 'Wednesday',
        4: 'Thursday',
        5: 'Friday',
        6: 'Saturday',
        7: 'Sunday',
      },
      weekdayShort: {
        1: 'Mon',
        2: 'Tue',
        3: 'Wed',
        4: 'Thu',
        5: 'Fri',
        6: 'Sat',
        7: 'Sun',
      },
      history: {
        none: 'No activity',
        partial: 'Partial',
        complete: 'Complete',
        dayActivity: '{{date}}: {{activity}}',
        dayProgress: '{{activity}}, {{completed}} of {{total}} habits completed',
      },
    },
    todos: {
      eyebrow: 'Tasks',
      title: 'Tasks',
      description: 'Organize tasks by priority, due date, and status without losing accidentally deleted items.',
      loading: 'Loading tasks...',
      empty: 'No tasks yet.',
      loadError: 'Could not load tasks.',
      saveError: 'Could not save task changes.',
      sorts: {
        priority: 'Priority',
        dueAt: 'Date',
        createdAt: 'Created',
      },
      modes: {
        list: 'List',
        calendar: 'Calendar',
      },
      priorities: {
        1: 'Important',
        2: 'Relevant',
        3: 'Low priority',
      },
      form: {
        titleLabel: 'Task title',
        titlePlaceholder: 'New task',
        descriptionLabel: 'Description',
        descriptionPlaceholder: 'Optional note',
        invalidDate: 'Use the YYYY-MM-DD format.',
        dueDateLabel: 'Due date',
        create: 'Create task',
        createTitle: 'New task',
        editTitle: 'Edit task',
      },
      actions: {
        save: 'Save',
        cancel: 'Cancel',
        reopen: 'Reopen',
        complete: 'Complete',
        edit: 'Edit',
        delete: 'Delete',
      },
      detail: {
        date: 'Date',
        priority: 'Priority',
        status: 'Status',
      },
      status: {
        completed: 'Completed',
        pending: 'Pending',
      },
      editLabel: 'Edit task',
      calendarNoDate: 'No date',
      noDueDate: 'No due date',
      dueToday: 'Due today',
      due: 'Due {{date}}',
      overdue: 'Overdue {{date}}',
      completed: 'Completed {{date}}',
    },
    settings: {
      eyebrow: 'Product',
      title: 'Settings',
      description: 'Local preferences, privacy, and product information for Done Loop.',
      loading: 'Loading settings...',
      preferences: 'Preferences',
      notifications: 'Notifications',
      notificationDetail: 'Local reminders for habits and tasks',
      animations: 'Animations',
      animationsDetail: 'Smooth motion across screens and actions',
      customization: 'Customization',
      theme: {
        section: 'Theme',
        system: 'System',
        light: 'Light',
        dark: 'Dark',
      },
      accentColor: {
        section: 'App color',
        purple: 'Purple',
        blue: 'Blue',
        green: 'Green',
        red: 'Red',
        yellow: 'Yellow',
        pink: 'Pink',
      },
      appBackground: {
        section: 'App background',
        none: 'No background',
        gradient: 'Gradient',
        grid: 'Grid',
        solar: 'Solar',
      },
      language: {
        section: 'Language',
        en: 'English',
        es: 'Spanish',
      },
      dateFormat: {
        section: 'Date format',
        iso: 'YYYY-MM-DD',
        mdy: 'MM/DD/YYYY',
        dmy: 'DD/MM/YYYY',
        long: 'Month D, YYYY',
      },
      information: 'Information',
      version: 'Version',
      privacy: 'Privacy',
      privacyPolicy: 'Privacy Policy',
      terms: 'Terms',
      termsOfService: 'Terms of Service',
      sourceCode: 'Source code',
      openLinkError: 'Could not open this link.',
      pending: 'Pending',
      sendFeedback: 'Send feedback',
      reportBugs: 'Report bugs',
    },
    notifications: {
      habitBody: 'Time to complete: {{habitName}}',
      markHabitComplete: 'Mark complete',
      remindIn30Minutes: 'Remind in 30 minutes',
      todoTitle: 'Upcoming task',
    },
    calendar: {
      eyebrow: 'Calendar',
      title: 'Calendar',
      description: 'Review habit activity and task dates in one monthly view.',
      habits: 'Habit activity',
      tasks: 'Tasks',
      noTasks: 'No tasks for this month.',
      noTasksForDay: 'No tasks for this day.',
      noHabitsForDay: 'No habits scheduled for this day.',
      selectedDay: 'Selected {{date}}',
      clearSelection: 'Clear selected day',
      previousMonth: 'Previous month',
      nextMonth: 'Next month',
    },
  },
  es: {
    common: {
      off: 'Off',
      on: 'On',
      future: 'futuro',
      pending: 'Pendiente',
      yes: 'Sí',
      no: 'No',
      none: 'Ninguna',
      clear: 'Limpiar',
      close: 'Cerrar',
      create: 'Crear',
    },
    tabs: {
      habits: 'Hábitos',
      todos: 'Tareas',
      settings: 'Ajustes',
      calendar: 'Calendario',
    },
    habits: {
      title: 'Hábitos',
      description: 'Crea hábitos diarios, marca el progreso de hoy y desactiva lo que ya no forma parte de tu rutina.',
      completedSummary: 'completados',
      pendingSummary: 'pendientes',
      loading: 'Cargando hábitos...',
      empty: 'Aún no tienes hábitos activos. Crea el primero para empezar el loop de hoy.',
      emptyFilter: 'No hay hábitos en este filtro.',
      loadError: 'No se pudieron cargar los hábitos.',
      saveError: 'No se pudieron guardar los cambios del hábito.',
      formLabel: 'Nombre del hábito',
      formPlaceholder: 'Nuevo hábito',
      descriptionLabel: 'Descripción',
      descriptionPlaceholder: 'Nota opcional',
      recurrence: 'Recurrencia',
      customInterval: 'Cada N días',
      customIntervalRequired: 'Ingresa un número entero mayor que cero.',
      customIntervalUnit: 'días',
      everyDays: 'Cada {{count}} días',
      weeklyDays: 'Días de la semana',
      monthlyDays: 'Días del mes',
      addMonthlyDays: 'Seleccionar días del mes',
      monthlyDateLabel: 'Fecha mensual {{index}}',
      removeMonthlyDate: 'Eliminar fecha mensual {{index}}',
      reminder: 'Recordatorio',
      reminderTime: 'Hora del recordatorio',
      create: 'Crear',
      createTitle: 'Nuevo hábito',
      editTitle: 'Editar hábito',
      editLabel: 'Editar hábito',
      completed: 'Completado',
      completedToday: 'Completado hoy',
      pending: 'Pendiente',
      pendingToday: 'Pendiente hoy',
      notScheduledToday: 'No toca hoy',
      save: 'Guardar',
      cancel: 'Cancelar',
      edit: 'Editar',
      delete: 'Eliminar',
      archived: 'Archivado',
      archivedHelp: 'Los hábitos archivados quedan inactivos hasta que los desarchives.',
      deleteConfirmTitle: '¿Eliminar hábito?',
      deleteConfirmMessage: 'Elige si quieres conservar el historial anterior o eliminar todos los registros de este hábito.',
      deleteFromDate: 'Eliminar desde este día en adelante',
      deleteAllHistory: 'Eliminar todo el historial',
      detail: {
        interval: 'Intervalo',
        recurrence: 'Recurrencia',
        days: 'Días',
        dates: 'Fechas',
        reminder: 'Recordatorio',
        reminderTime: 'Hora',
        status: 'Estado',
      },
      filters: {
        all: 'Todos',
        pendingToday: 'Pendientes',
        completedToday: 'Listos',
      },
      recurrences: {
        daily: 'Diario',
        weekly: 'Semanal',
        monthly: 'Mensual',
        custom: 'Personalizado',
      },
      weekdays: {
        1: 'Lunes',
        2: 'Martes',
        3: 'Miércoles',
        4: 'Jueves',
        5: 'Viernes',
        6: 'Sábado',
        7: 'Domingo',
      },
      weekdayShort: {
        1: 'Lun',
        2: 'Mar',
        3: 'Mié',
        4: 'Jue',
        5: 'Vie',
        6: 'Sáb',
        7: 'Dom',
      },
      history: {
        none: 'Sin actividad',
        partial: 'Parcial',
        complete: 'Completa',
        dayActivity: '{{date}}: {{activity}}',
        dayProgress: '{{activity}}, {{completed}} de {{total}} hábitos completados',
      },
    },
    todos: {
      eyebrow: 'Tareas',
      title: 'Tareas',
      description: 'Organiza pendientes por prioridad, vencimiento y estado sin perder tareas eliminadas por accidente.',
      loading: 'Cargando tareas...',
      empty: 'Aún no hay tareas.',
      loadError: 'No se pudieron cargar las tareas.',
      saveError: 'No se pudieron guardar los cambios de la tarea.',
      sorts: {
        priority: 'Prioridad',
        dueAt: 'Fecha',
        createdAt: 'Creación',
      },
      modes: {
        list: 'Lista',
        calendar: 'Calendario',
      },
      priorities: {
        1: 'Importante',
        2: 'Relevante',
        3: 'Sin importancia',
      },
      form: {
        titleLabel: 'Título de la tarea',
        titlePlaceholder: 'Nueva tarea',
        descriptionLabel: 'Descripción',
        descriptionPlaceholder: 'Nota opcional',
        invalidDate: 'Usa el formato YYYY-MM-DD.',
        dueDateLabel: 'Fecha límite',
        create: 'Crear tarea',
        createTitle: 'Nueva tarea',
        editTitle: 'Editar tarea',
      },
      actions: {
        save: 'Guardar',
        cancel: 'Cancelar',
        reopen: 'Reabrir',
        complete: 'Completar',
        edit: 'Editar',
        delete: 'Eliminar',
      },
      detail: {
        date: 'Fecha',
        priority: 'Prioridad',
        status: 'Estado',
      },
      status: {
        completed: 'Completada',
        pending: 'Pendiente',
      },
      editLabel: 'Editar tarea',
      calendarNoDate: 'Sin fecha',
      noDueDate: 'Sin fecha límite',
      dueToday: 'Vence hoy',
      due: 'Vence {{date}}',
      overdue: 'Vencida {{date}}',
      completed: 'Completada {{date}}',
    },
    settings: {
      eyebrow: 'Producto',
      title: 'Ajustes',
      description: 'Preferencias locales, privacidad e información del producto para Done Loop.',
      loading: 'Cargando ajustes...',
      preferences: 'Preferencias',
      notifications: 'Notificaciones',
      notificationDetail: 'Recordatorios locales para hábitos y tareas',
      animations: 'Animaciones',
      animationsDetail: 'Movimiento suave entre pantallas y acciones',
      customization: 'Personalización',
      theme: {
        section: 'Tema',
        system: 'Sistema',
        light: 'Claro',
        dark: 'Oscuro',
      },
      accentColor: {
        section: 'Color de la app',
        purple: 'Morado',
        blue: 'Azul',
        green: 'Verde',
        red: 'Rojo',
        yellow: 'Amarillo',
        pink: 'Rosado',
      },
      appBackground: {
        section: 'Fondo de la app',
        none: 'Sin fondo',
        gradient: 'Gradiente',
        grid: 'Grid',
        solar: 'Solar',
      },
      language: {
        section: 'Idioma',
        en: 'Inglés',
        es: 'Español',
      },
      dateFormat: {
        section: 'Formato de fecha',
        iso: 'YYYY-MM-DD',
        mdy: 'MM/DD/YYYY',
        dmy: 'DD/MM/YYYY',
        long: 'Mes D, YYYY',
      },
      information: 'Información',
      version: 'Versión',
      privacy: 'Privacidad',
      privacyPolicy: 'Política de Privacidad',
      terms: 'Términos',
      termsOfService: 'Términos de Servicio',
      sourceCode: 'Código fuente',
      openLinkError: 'No se pudo abrir este enlace.',
      pending: 'Pendiente',
      sendFeedback: 'Enviar feedback',
      reportBugs: 'Reportar errores',
    },
    notifications: {
      habitBody: 'Hora de completar: {{habitName}}',
      markHabitComplete: 'Marcar completado',
      remindIn30Minutes: 'Recordar en 30 minutos',
      todoTitle: 'Tarea próxima',
    },
    calendar: {
      eyebrow: 'Calendario',
      title: 'Calendario',
      description: 'Revisa actividad de hábitos y fechas de tareas en una vista mensual.',
      habits: 'Actividad de hábitos',
      tasks: 'Tareas',
      noTasks: 'No hay tareas para este mes.',
      noTasksForDay: 'No hay tareas para este día.',
      noHabitsForDay: 'No hay hábitos programados para este día.',
      selectedDay: 'Seleccionado {{date}}',
      clearSelection: 'Limpiar día seleccionado',
      previousMonth: 'Mes anterior',
      nextMonth: 'Mes siguiente',
    },
  },
} as const;

export type Language = keyof typeof translations;
export type TranslationKey = string;
